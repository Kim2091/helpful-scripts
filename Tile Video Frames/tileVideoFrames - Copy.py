...
def process_scene(scene_name, scene_frames, output_dir, sequence_length, tile_width, tile_height, global_show_counter, prefix, num_width):
    """
    ...
    for seq_start_idx in range(num_sequences):
        ...
        images = []
        for frame_num, filepath, filename in sequence:
            images.append(Image.open(filepath))
        
        # Extract tiles from each position
        for tile_idx, (x, y) in enumerate(tile_positions):
            # Extract tile from each frame in the sequence
            for frame_idx in range(sequence_length):
                img = images[frame_idx]
                tile = extract_tile(img, x, y, tile_width, tile_height)
                
                # Generate output filename with show number and frame number
                # Frame numbers reset for each tile sequence
                output_filename = f"{prefix}{global_show_counter:0{num_width}d}_Frame{frame_idx + 1:05d}{extension}"
                output_path = os.path.join(output_dir, output_filename)
                
                tile.save(output_path)
            
            # Increment show counter after each tile sequence
            global_show_counter += 1
        
        # Close all images in the sequence
        for img in images:
            img.close()
        
        print(f"  Sequence {seq_start_idx + 1}/{num_sequences}: {len(tile_positions)} tiles × {sequence_length} frames = {len(tile_positions) * sequence_length} output frames")
...
def main():
    ...
    for scene_name in sorted(scenes.keys()):
        ...
        global_show_counter = process_scene(scene_name, scene_frames, args.output_dir, 
                                            ...
                                            args.tile_height, global_show_counter, prefix, num_width)
