...
class ImageComparator:
    ...
    def overlay_images(self, img1_path, img2_path, dest_path):
        try:
            ...
            if img1 is None or img2 is None:
                logging.warning(f'Failed to load images for overlay: {img1_path}, {img2_path}')
                return False
            overlay = cv2.addWeighted(img1, 0.5, img2, 0.5, 0)
            cv2.imwrite(dest_path, overlay)
            return True
        except Exception as e:
            logging.error(f'Error creating overlay for {img1_path} and {img2_path}: {e}')
            return False
    ...
    def compare_images(self, img1_path, img2_path):
        try:
            ...
            if img1 is None or img2 is None:
                logging.warning(f'Failed to load images: {img1_path}, {img2_path}')
                return None
    ...
    def process_image(self, filename, folder1, folder2, dest_folder):
        ...
        if score < self.threshold:
            ...
            try:
                ...
                with self.moved_lock:
                    self.moved_count += 1
                
                logging.info(f'Moved {filename} from {img1_path} and {img2_path} to {dest_folder} due to low similarity score')
            except Exception as e:
                logging.error(f'Error moving files {img1_path} and {img2_path}: {e}')
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                for dirpath, _, filenames in os.walk(folder1):
                    ...
                    if os.path.exists(dirpath2):
                        # Submit tasks without forcing eager evaluation
                        for filename in filenames:
                            executor.submit(self.process_image, filename, dirpath, dirpath2, dest_folder)
...
if comparator.moved_count > 0:
    logging.info(f"Total images moved: {comparator.moved_count}")
else:
